...
namespace NaughtyBezierCurves.Editor
{
    ...
    public class BezierCurve3DEditor : UnityEditor.Editor
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            this.keyPoints = new ReorderableList(this.serializedObject, serializedObject.FindProperty("keyPoints"), true, true, false,
                false);
            this.keyPoints.drawElementCallback = this.DrawElementCallback;
            this.keyPoints.drawHeaderCallback =
                (Rect rect) =>
                {
                    EditorGUI.LabelField(rect, string.Format("Points: {0}", this.keyPoints.serializedProperty.arraySize),
        ...
        private void DrawElementCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            EditorGUI.PropertyField(
                new Rect(rect.x + AddButtonWidth + 5f, rect.y, rect.width - AddButtonWidth * 2f - 35f, EditorGUIUtility.singleLineHeight),
                element, GUIContent.none);

            // Draw "Add After" button
            if (GUI.Button(new Rect(rect.width - AddButtonWidth + 8f, rect.y, AddButtonWidth, EditorGUIUtility.singleLineHeight),
                new GUIContent("Add After")))
            {
        ...
        private static BezierPoint3D AddKeyPointAt(BezierCurve3D curve, int index)
        {
            ...
            else
            {
                ...
                {
                    newPoint.Position = (curve.KeyPoints[index - 1].Position - curve.KeyPoints[index - 2].Position).normalized +
